...
- Stream content: `Hello{version, manifest, world hash, channels}`, `Keyframe{t, all actor poses (`i32` mm east and north and `i16` cm up, about an origin carried in every keyframe and held constant for the run; heading in `u16` binary radians), node summaries, signal states}` every K seconds (default 1 s), `Delta{t, changed poses (`i16` mm on all three axes, taken against the previously *transmitted* quantised value so the error stays bounded instead of accumulating, with an absolute-escape flag for teleports), events since last delta}`, `Telemetry{node, sampled counters}`, `Metric{samples}`, `Provenance{ids → (model, params)}` on demand.
