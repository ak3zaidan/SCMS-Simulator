...
- Transport: WebSocket; binary frames are FlatBuffers tables from `ui/packages/protocol` (generated from the same `.fbs` files the recorder uses); a JSON-RPC 2.0 control channel (same socket, text frames, or HTTP POST) carries commands: `run.start`, `run.pause`, `run.step`, `run.seek`, `run.speed`, `view.follow`, `overlay.set`, `inspect`, `explain`, `config.get/set/validate`, `experiment.*`, `export.*`.
