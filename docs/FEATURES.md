...
| Manifest | `manifest.json` records seed, full config, per-file SHA-256, data digest, schema versions, the interpreter/host `runtime` block (`sys.version`, platform, `hash_randomization`), the `plugins` lock (what was actually loaded, content-addressed), and a standards profile | (every run) |
| Standards profile | **Implemented:** CAMP SCP2 linkage; IEEE 1609.2 §6.4.3 HashedId8 *identifiers*. **Inspired-by:** TS 103 759 report field names (partial). **Absent:** all message encodings, all security envelopes, all certificate structures — no signature is computed or verified (`sig_ok` is simulated). The `cert: IEEE 1609.2` claim was withdrawn 2026-08-30 as unsupportable | (every run) |
| Component plugins | Channel/radio model is a published contract (`scms_sim_ref.api`), not a closed enum: `disc`/`logdistance`/`geometric` are built-in registry entries, and a third party selects their own via built-in name, entry point or dotted path `pkg.mod:Class` with **zero engine edits**. Activation is always config-declared; empty (the default) is byte-identical | `--plugins '{"channel_model":{"ref":...}}'` |
| Plugin provenance lock | `manifest["plugins"]` records what was ACTUALLY loaded — ref, resolution tier, interface version, declared capabilities and RNG stream labels, `module_sha256`/`dist_sha256`/`params_sha256` and a `provenance_digest`. Replaying a manifest whose plugin source changed fails **before step 0**, exit 2; `--allow-plugin-drift` proceeds and **records** the drift as `plugins.drift_allowed` in the new manifest rather than silencing it | `--allow-plugin-drift` |
| Plugin conformance suite | Versioned checks C1–C12 (`scms_sim_ref.conformance`, suite `v1`, graded against `ChannelModel/1.x`): repeatability, call-order independence, global-RNG untouched, per-step state advances exactly once, no I/O, **oracle invariance**, output ranges, reach honesty, monotone in distance, fail-fast params, float hygiene, two-run pipeline digest. Runs as a CLI *or* as pytest tests by subclassing `ChannelModelContract`. Waivers are data the implementation ships, with a written justification carried into the report | `conformance --slot channel_model --ref pkg.mod:Class [--report conf.json]` |
| Plugin attestation (opt-in) | `plugins.<slot>.conformance = "required"` makes the engine run the suite **before step 0**, refuse a non-conformant plugin naming the failed checks, and embed the summary in `manifest["plugins"]["loaded"][*]["conformance"]`. measured 0.11 s per run; zero effect on `data_digest`. C12 is excluded before the suite runs (it runs two pipelines, so running it inside one nests a pipeline in a pipeline) and the exclusion is named in the embedded summary | `--plugins '{"channel_model":{"ref":...,"conformance":"required"}}'` |
| Plugin lock verification | Re-resolve a manifest's plugin lock and recompute its content hashes with **no simulation** — a CI gate. Exit 0 clean, 2 on drift or an edited lock. `tools/verify_data.py` additionally audits the lock on disk (`PL1`–`PL6`) | `verify-plugins manifest.json [--allow-drift] [--json]` |
