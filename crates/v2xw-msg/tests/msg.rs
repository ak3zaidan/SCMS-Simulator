...
use v2xw_msg::{BsmGenerator, CamGenerator, EtsiUperCodec, J2735BsmCodec, J2735SizeCodec};
...
fn the_registered_codecs_partition_the_message_set_they_claim() {
    ...
    let bsm = J2735BsmCodec::new();
    let modelled = J2735SizeCodec::new();

    assert_eq!(etsi.family(), Family::Codec);
    assert_eq!(bsm.family(), Family::Codec);
    assert_eq!(modelled.family(), Family::Codec);

    for ty in MsgType::ALL {
        let claims = [etsi.supports(ty), bsm.supports(ty), modelled.supports(ty)];
        assert!(
            claims.iter().filter(|c| **c).count() <= 1,
            "{ty} is claimed by more than one codec"
        );
    }

    // And the ones nobody claims are exactly the ones a later stage owns.
    let unclaimed: Vec<MsgType> = MsgType::ALL
        .into_iter()
        .filter(|ty| !etsi.supports(*ty) && !bsm.supports(*ty) && !modelled.supports(*ty))
        .collect();
    assert_eq!(
        unclaimed,
        vec![
            MsgType::Vam, // generates cleanly; not yet in the FACILITIES unit
            MsgType::Cpm, // likewise
...
fn the_size_model_returns_placeholders_that_cannot_be_decoded() {
    ...
    for ty in [
        ...
        let encoded = codec.encode(&Message::Modeled(request)).expect("sizes");
