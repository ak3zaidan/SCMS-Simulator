...
import { PHASE_NO_DATA, aspectOf, signalGroupWireId } from "../src/signals.js";
...
describe("signal heads", () => {
  ...
  it("lights only a group's own heads from a group row, and every head from a controller row", () => {
    // The engine streams one row per signal group, id (controller + 1) * 65536 + group (vwp-v1
    // 3.3.3), so the two cross streets of a junction can show different states at once.
    const w = new WorldRenderer({ theme: DARK_THEME });
    w.setWorld(grid.world);
    const heads = headsOf(w, 0);
    expect(heads.length).toBe(4);
    w.applySignalKeyframe(block([
      [signalGroupWireId(0, 0), 6], [signalGroupWireId(0, 1), 3],
      [signalGroupWireId(0, 2), 6], [signalGroupWireId(0, 3), 3],
    ]));
    const names = heads.map((h) => w.signals.headState(h)?.aspect.name);
    expect(names).toEqual(["green", "red", "green", "red"]);
    // A plain controller row (the mock server's) still colours all of them.
    w.applySignalDelta(block([[0, 8]]));
    for (const h of heads) expect(w.signals.headState(h)?.aspect.name).toBe("amber");
    w.dispose();
  });
